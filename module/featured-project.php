
<!-- featured project -->
	<div class="container">
		<div class="featured-project">
			<div class="row">
				<div class="span12">
					<div class="row">
						<div class="span6">
							<div id="img-featured">
								<a href="taxonomy-project.php"><img src="upload/featured-project2.jpg" alt="twitgreen"></a>
							</div>
						</div>
						<div class="span6">
							<div id="text-featured">
							<h1 class="large">
								<a href="taxonomy-project.php">Menabung 100.000.000 Pohon Pertamina Sobat Bumi</a>
							</h1>
							<p class="big">
								PT. Pertamina (Persero) bercita-cita menanam dan merawat 100 juta pohon di seluruh Indonesia hingga tahun 2015.
								Program ini salah satu upaya yang dilakukan oleh PT Pertamina (Persero)  [...]
							</p>
							</div>
							<div class="total">
								<p class="big">
									Saat ini sudah Menabung <a href="#">26.000.000</a> Pohon
								</p>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
	<!-- /featured project -->